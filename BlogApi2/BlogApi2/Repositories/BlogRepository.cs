...
namespace BlogApi2.Repositories
{
    public class BlogRepository : IBlogRepository
    {
        ...
        public async Task<bool> DeleteBlog(string id)
        {
            ...
            DeleteResult deleteResult = await _blogContext.Blogs .DeleteOneAsync(filter);
        ...
        {
            return await _blogContext.Blogs.Find(p => p.Id == id).FirstOrDefaultAsync();
        }
        ...
        public async Task<IEnumerable<Blog>> GetBlogByTitle(string name)
        {
            ...
            return await _blogContext.Blogs.Find(filter).ToListAsync();
        }

        public async Task<IEnumerable<Blog>> GetBlogs()
        {
            return await _blogContext.Blogs.Find(p => true).ToListAsync();
        }

        public async Task<bool> UpdateBlog(Blog blog)
        {
            var updateResult = await _blogContext.Blogs.ReplaceOneAsync(filter: g => g.Id == blog.Id, replacement: blog);
